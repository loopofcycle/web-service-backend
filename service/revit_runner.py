...
class RevitRunner(NameSpaceConfig):
    ...
    async def run_process(self) -> dict:
        ...
        with open(self.config.revit_config_path, 'w', encoding='utf-8') as json_file:
            json.dump(self.revit_config, json_file,
        ...
        # returning result
        return {'revit_result': self.log_message,
    ...
    async def wait_for_process(self) -> str:
        ...
        async def coro_read_stdout() -> str:
            ...
            for line in io.TextIOWrapper(self.process.stdout, encoding=preferred_encoding):

                self.return_code = self.process.poll()
                if self.return_code is not None:
                    self.proc_status = ProcessStatus.FAILED
                    self.log_message = f'process was shut down, returncode {hex(self.return_code)}'
                    self.log(
        ...
        try:
            ...
            if self.process.poll() is None:
                ...
                self.log(
                    f'[pid={self.process.pid}] terminated: {self.log_message}')
            ...
            self.proc_status = ProcessStatus.FAILED
            self.log(
                f'process pid={self.process.pid} was terminated due to timeout')
